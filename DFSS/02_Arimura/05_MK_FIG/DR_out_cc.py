...
def c_cc(sig,rho,tant,tanp):
    if rho<=0.01: cc=0.
    else: cc=tant/(tanp-tant)/(sig/rho-1.)
    return cc
rho=[c_rho(x1/g,tant,tanp) for x1 in rm_obs]
cc=[c_cc(sig,x1,tant,tanp)for x1 in rho]
plt.plot(time[:-1],cc[1:],ls='--',color='k',lw=0.6,label=r'${c_{c} (obs.)}$')
######################################################
loc=sys.argv[1:]
...
for tdir in dirs:
    ...
    for l in loc:
        ...
        q=x[4]
        c=x[5]
        #plt.plot(time,u,lw=0.6,ls='-',color='gray',label=r'$\it{u (sim)}$')
        #plt.plot(time,h,lw=0.6,ls='-',color='k',label=r'$\it{h (sim)}$')
        plt.plot(time,c,lw=0.6,color='k',label=r'$\it{c_{c} (sim.)}$')
plt.xlim(0,25)
plt.ylim(0,0.3)
plt.xlabel('Time (min)')
plt.ylabel(r'$\it{c_{c}}$')
#plt.yscale('log')
plt.tight_layout()
#plt.legend(bbox_to_anchor=(0.97, 0.87),fontsize=fsize*0.8)
plt.legend(loc='upper right',fontsize=fsize*0.8)
plt.savefig('DR_cc.png',dpi=300)
#plt.show()
