...
#
ls='\n'
tpath=os.path.join('..','..','04_EXE_SIMU','01_RR_DR')
fo_h='DR_{0:010d}.vtk'
files=glob.glob(os.path.join(tpath,'output_DR','res_??????????.txt'))
#
fvtk=open('stream_line.vtk','w')
fn=os.path.join(tpath,'input','streamConfiguration_inRR.txt')
...
for fn in files:
    ...
    if sec%dt_out==0:
        ...
        za=data.T[1].astype(float)
        ha=data.T[2].astype(float)
        ua=data.T[3].astype(float)
        qa=data.T[4].astype(float)
        cca=data.T[5].astype(float)
        cfa=data.T[6].astype(float)
        z0a=data.T[9].astype(float)
        dza=za-z0a
        ...
        for cvar,var in [['h',ha],['u',ua],['q',qa],['cc',cca],['cf',cfa],['dz',dza]]:
            vtk.write(ls)
