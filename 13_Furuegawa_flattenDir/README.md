...
`fortran` コンパイラして、オープンな `gfortran` や `LLVM` を用いた `lfort`， 無償で利用できる `Intel oneAPI HPC Toolkit` の `ifort` (将来は`ifx`に変更予定) があります。
...
他の GIS ソフトウェアの処理では流向の定義の違い適切に動作しません。
...
なお，`macOS` や `Linux` における `zsh` 等では環境変数の設定方法が異なります（`OMP_NUM_THREADS=8` とする）。
...
オリジナルデータ `DFSS/03_Furuegawa/05_MK_FIG` の `python` による可視化用のコードを使いやすいように改良し `./bin_fig` に集めた。 `MK_riverNodes.py` は引数に与える値を調べられるように新たに作成したものです。
可視化用のコードは `--help` もしくは `-h` の引数を与えると使い方の詳細が分かります（例えば、 `python bin_fig\RR_out_hyeto.py --help` とする）。
...
あらかじめ以下の `python` ライブラリのインストールしてください。
...
プロキシが設定されている環境下では、以下のようにあらかじめ設定してから上のコマンドを打ち込みます（`Windows`における`cmd.exe` の場合）。
...
第2引数は、国土地理院が定めた日本測地系2011 `JGD2011`の緯度経度座標系であれば `EPSG:6668` 、日本測地系2000 `JGD2000` であれば `EPSG:4612` とする。

地理院地図で、`./river_node_lonlat.kml` を表示して、節点（ノード）をクリックすると、以下の図に示すように節点番号を得ることができる。

![](./img/river_node_lowerest.png width=360px)

![](./img/river_node_outlet.png width=360px)
