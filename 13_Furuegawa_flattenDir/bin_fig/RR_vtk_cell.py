def make_RR_vtk_cell(SKIP=1):
    ...
    srcpath = os.getcwd()
    varis=('h','hs')
    fo_h='RR_cell_{}_{:010d}.vtk'
    fz=os.path.join(srcpath,'elevation_inRR.asc')
    fb=os.path.join(srcpath,'targetArea_inRR.asc')
    files0=glob.glob(os.path.join(srcpath,'output_RR','res_*_??????????.asc'))
    files0.sort()
    #
...
if args.skip > 0:
    make_RR_vtk_cell(SKIP=args.skip)
else:
    import sys
